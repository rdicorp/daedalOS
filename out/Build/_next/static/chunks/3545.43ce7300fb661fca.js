...
(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
    [3545],
    {
        53545: function (e, t, i) {
            i.d(t, {
                fileTypeFromBuffer: function () {
                    return S;
                },
            }),
                i(58090);
            ...
            i(80645);
            let s = { len: 1, get: (e, t) => n(e).getUint8(t), put: (e, t, i) => (n(e).setUint8(t, i), t + 1) },
                a = { len: 2, get: (e, t) => n(e).getUint16(t, !0), put: (e, t, i) => (n(e).setUint16(t, i, !0), t + 2) },
                f = { len: 2, get: (e, t) => n(e).getUint16(t), put: (e, t, i) => (n(e).setUint16(t, i), t + 2) },
                c = { len: 4, get: (e, t) => n(e).getUint32(t, !0), put: (e, t, i) => (n(e).setUint32(t, i, !0), t + 4) },
                o = { len: 4, get: (e, t) => n(e).getUint32(t), put: (e, t, i) => (n(e).setUint32(t, i), t + 4) },
                h = { len: 4, get: (e, t) => n(e).getInt32(t), put: (e, t, i) => (n(e).setInt32(t, i), t + 4) },
                m = { len: 8, get: (e, t) => n(e).getBigUint64(t, !0), put: (e, t, i) => (n(e).setBigUint64(t, i, !0), t + 8) };
            class u {
                constructor(e, t) {
                    (this.len = e), (this.encoding = t);
                }
                get(e, t) {
                    return r.Buffer.from(e).toString(this.encoding, t, t + this.len);
                }
            }
            class p {
                constructor(e) {
                ...
                static decode(e, t, i) {
                    let r = "";
                    for (let n = t; n < i; ++n) r += p.codePointToString(p.singleByteDecoder(e[n]));
                    return r;
                }
                ...
                static singleByteDecoder(e) {
                    if (p.inRange(e, 0, 127)) return e;
                    let t = p.windows1252[e - 128];
                    if (null === t) throw Error("invaliding encoding");
                    return t;
                }
                get(e, t = 0) {
                    return p.decode(e, t, t + this.len);
                }
            }
            p.windows1252 = [
                8364,
            ...
            class x {
                constructor(e) {
                    ...
                    this.s.once("end", () => this.reject(new l())), this.s.once("error", (e) => this.reject(e)), this.s.once("close", () => this.reject(Error("Stream closed")));
                }
                async peek(e, t, i) {
                    let r = await this.read(e, t, i);
                    return this.peekQueue.push(e.subarray(t, t + r)), r;
                }
                async read(e, t, i) {
                    if (0 === i) return 0;
                    if (0 === this.peekQueue.length && this.endOfStream) throw new l();
                    let r = i,
                        n = 0;
                    for (; this.peekQueue.length > 0 && r > 0;) {
                        let i = this.peekQueue.pop();
                        if (!i) throw Error("peekData should be defined");
                        let s = Math.min(i.length, r);
                        e.set(i.subarray(0, s), t + n), (n += s), (r -= s), s < i.length && this.peekQueue.push(i.subarray(s));
                    }
                    for (; r > 0 && !this.endOfStream;) {
                        let i = Math.min(r, 1048576),
                            s = await this.readFromStream(e, t + n, i);
                        if (((n += s), s < i)) break;
                        r -= s;
                    }
                    return n;
                ...
                async readFromStream(e, t, i) {
                    ...
                    {
                        let r = { buffer: e, offset: t, length: i, deferred: new d() };
                        return (
            ...
            class g {
                ...
                async readToken(e, t = this.position) {
                    let i = r.Buffer.alloc(e.len);
                    if ((await this.readBuffer(i, { position: t })) < e.len) throw new l();
                    return e.get(i, 0);
                }
                async peekToken(e, t = this.position) {
                    let i = r.Buffer.alloc(e.len);
                    if ((await this.peekBuffer(i, { position: t })) < e.len) throw new l();
                    return e.get(i, 0);
                }
                async readNumber(e) {
            ...
            class k extends g {
                constructor(e, t) {
                    super(t), (this.streamReader = new x(e));
                }
                async getFileInfo() {
                    return this.fileInfo;
                }
                async readBuffer(e, t) {
                    let i = this.normalizeOptions(e, t),
                    ...
                    let n = await this.streamReader.read(e, i.offset, i.length);
                    if (((this.position += n), (!t || !t.mayBeLess) && n < i.length)) throw new l();
                    return n;
                }
                async peekBuffer(e, t) {
                    let i = this.normalizeOptions(e, t),
                        r = 0;
                    if (i.position) {
                        let t = i.position - this.position;
                        if (t > 0) {
                            let n = new Uint8Array(i.length + t);
                    ...
                    if (i.length > 0) {
                        try {
                            ...
                            if (t && t.mayBeLess && e instanceof l) return 0;
                            throw e;
                        }
                        if (!i.mayBeLess && r < i.length) throw new l();
                    }
                    return r;
                }
                async ignore(e) {
                    ...
                    for (; r < e;) {
                        let n = e - r,
                            s = await this.readBuffer(i, { length: Math.min(t, n) });
                        if (s < 0) return s;
                        r += s;
                    }
                    return r;
            ...
            class w extends g {
                ...
                async readBuffer(e, t) {
                    ...
                    let i = await this.peekBuffer(e, t);
                    return (this.position += i), i;
                }
                async peekBuffer(e, t) {
                    let i = this.normalizeOptions(e, t),
                        r = Math.min(this.uint8Array.length - i.position, i.length);
                    if (i.mayBeLess || !(r < i.length)) return e.set(this.uint8Array.subarray(i.position, i.position + r), i.offset), r;
                    throw new l();
                }
                async close() { }
            }
            let b = { get: (e, t) => (127 & e[t + 3]) | (e[t + 2] << 7) | (e[t + 1] << 14) | (e[t] << 21), len: 4 };
            async function S(e) {
                return new v().fromBuffer(e);
            }
            function y(e, t, i) {
                for (let [r, n] of ((i = { offset: 0, ...i }), t.entries()))
                    if (i.mask) {
            ...
            class v {
                constructor(e) {
                    (this.detectors = e?.customDetectors), (this.fromTokenizer = this.fromTokenizer.bind(this)), (this.fromBuffer = this.fromBuffer.bind(this)), (this.parse = this.parse.bind(this));
                }
                async fromTokenizer(e) {
                    let t = e.position;
                    for (let i of this.detectors || []) {
                        let r = await i(e);
                        if (r) return r;
                ...
                async fromBuffer(e) {
                    if (!(e instanceof Uint8Array || e instanceof ArrayBuffer)) throw TypeError(`Expected the \`input\` argument to be of type \`Uint8Array\` or \`Buffer\` or \`ArrayBuffer\`, got \`${typeof e}\``);
                    let t = e instanceof Uint8Array ? e : new Uint8Array(e);
                    if (t?.length > 1) return this.fromTokenizer(new w(t, void 0));
                }
                async fromBlob(e) {
                    let t = await e.arrayBuffer();
                    return this.fromBuffer(new Uint8Array(t));
                }
                async fromStream(e) {
                    var t;
                    let i = await new k(e, (t = t || {}));
                    try {
                        return await this.fromTokenizer(i);
                    } finally {
                        await i.close();
                    }
                }
                async toDetectionStream(e, t = {}) {
                    let { default: n } = await Promise.resolve().then(i.t.bind(i, 88473, 19)),
                        { sampleSize: s = 4100 } = t;
                    return new Promise((t, i) => {
                        e.on("error", i),
                            e.once("readable", () => {
                                (async () => {
                                    try {
                                        let a = new n.PassThrough(),
                                            f = n.pipeline ? n.pipeline(e, a, () => { }) : e.pipe(a),
                                            c = e.read(s) ?? e.read() ?? r.Buffer.alloc(0);
                                        try {
                                            a.fileType = await this.fromBuffer(c);
                                        } catch (e) {
                                            e instanceof l ? (a.fileType = void 0) : i(e);
                                        }
                                        t(f);
                                    } catch (e) {
                                        i(e);
                ...
                async parse(e) {
                    if (
                        ((this.buffer = r.Buffer.alloc(4100)),
                    ...
                    if (this.checkString("ID3")) {
                        await e.ignore(6);
                        let t = await e.readToken(b);
                        return e.position + t > e.fileInfo.size ? { ext: "mp3", mime: "audio/mpeg" } : (await e.ignore(t), this.fromTokenizer(e));
                    }
                    if (this.checkString("MP+")) return { ext: "mpc", mime: "audio/x-musepack" };
                    if ((67 === this.buffer[0] || 70 === this.buffer[0]) && this.check([87, 83], { offset: 1 })) return { ext: "swf", mime: "application/x-shockwave-flash" };
                    ...
                    if (this.check([80, 75, 3, 4])) {
                        try {
                            for (; e.position + 30 < e.fileInfo.size;) {
                                ...
                                let t = { compressedSize: this.buffer.readUInt32LE(18), uncompressedSize: this.buffer.readUInt32LE(22), filenameLength: this.buffer.readUInt16LE(26), extraFieldLength: this.buffer.readUInt16LE(28) };
                                if (((t.filename = await e.readToken(new u(t.filenameLength, "utf-8"))), await e.ignore(t.extraFieldLength), "META-INF/mozilla.rsa" === t.filename)) return { ext: "xpi", mime: "application/x-xpinstall" };
                                if (t.filename.endsWith(".rels") || t.filename.endsWith(".xml"))
                                    switch (t.filename.split("/")[0]) {
                                        case "_rels":
                                            break;
                                        case "word":
                                ...
                                if (t.filename.startsWith("xl/")) return { ext: "xlsx", mime: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" };
                                if (t.filename.startsWith("3D/") && t.filename.endsWith(".model")) return { ext: "3mf", mime: "model/3mf" };
                                if ("mimetype" === t.filename && t.compressedSize === t.uncompressedSize) {
                                    let i = await e.readToken(new u(t.compressedSize, "utf-8"));
                                    switch ((i = i.trim())) {
                                ...
                                if (0 === t.compressedSize) {
                                    let t = -1;
                                    for (; t < 0 && e.position < e.fileInfo.size;) await e.peekBuffer(this.buffer, { mayBeLess: !0 }), (t = this.buffer.indexOf("504B0304", 0, "hex")), await e.ignore(t >= 0 ? t : this.buffer.length);
                                } else await e.ignore(t.compressedSize);
                            }
                        } catch (e) {
                    ...
                    if (this.checkString("OggS")) {
                        await e.ignore(28);
                        let t = r.Buffer.alloc(8);
                        return (await e.readBuffer(t), y(t, [79, 112, 117, 115, 72, 101, 97, 100]))
                    ...
                    if (this.checkString("ftyp", { offset: 4 }) && (96 & this.buffer[8]) != 0) {
                        let e = this.buffer.toString("binary", 8, 12).replace("\0", " ").trim();
                        switch (e) {
                            case "avif":
                            case "avis":
                    ...
                    if (this.checkString("%PDF")) {
                        try {
                            await e.ignore(1350);
                            let t = r.Buffer.alloc(Math.min(10485760, e.fileInfo.size));
                            if ((await e.readBuffer(t, { mayBeLess: !0 }), t.includes(r.Buffer.from("AIPrivateData")))) return { ext: "ai", mime: "application/postscript" };
                        } catch (e) {
                            if (!(e instanceof l)) throw e;
                        }
                    ...
                    if (this.check([26, 69, 223, 163])) {
                        async function t() {
                            let t = await e.peekNumber(s),
                                i = 128,
                                n = 0;
                            for (; (t & i) == 0 && 0 !== i;) ++n, (i >>= 1);
                            let a = r.Buffer.alloc(n + 1);
                        ...
                        async function i() {
                            let e = await t(),
                                i = await t();
                            i[0] ^= 128 >> (i.length - 1);
                            let r = Math.min(6, i.length);
                            return { id: e.readUIntBE(0, e.length), len: i.readUIntBE(i.length - r, r) };
                        }
                        async function n(t) {
                            for (; t > 0;) {
                                let r = await i();
                                if (17026 === r.id) return (await e.readToken(new u(r.len, "utf-8"))).replaceAll(/\00.*$/g, "");
                    ...
                    if (this.check([82, 97, 114, 33, 26, 7]) && (0 === this.buffer[6] || 1 === this.buffer[6])) return { ext: "rar", mime: "application/x-rar-compressed" };
                    if (this.checkString("solid ")) return { ext: "stl", mime: "model/stl" };
                    if (this.checkString("AC")) {
                        let e = this.buffer.toString("binary", 2, 6);
                    ...
                    if (this.checkString("!<arch>")) return (await e.ignore(8), "debian-binary" === (await e.readToken(new u(13, "ascii")))) ? { ext: "deb", mime: "application/x-deb" } : { ext: "ar", mime: "application/x-unix-archive" };
                    if (this.checkString("**ACE", { offset: 7 }) && (await e.peekBuffer(this.buffer, { length: 14, mayBeLess: !0 }), this.checkString("**", { offset: 12 }))) return { ext: "ace", mime: "application/x-ace-compressed" };
                    if (this.check([137, 80, 78, 71, 13, 10, 26, 10])) {
                        async function a() {
                            return { length: await e.readToken(h), type: await e.readToken(new u(4, "binary")) };
                        }
                        await e.ignore(8);
                        do {
                            let t = await a();
                    ...
                    if (this.check([48, 38, 178, 117, 142, 102, 207, 17, 166, 217])) {
                        async function f() {
                            let t = r.Buffer.alloc(16);
                            return await e.readBuffer(t), { id: t, size: Number(await e.readToken(m)) };
                        }
                        for (await e.ignore(30); e.position + 24 < e.fileInfo.size;) {
                            let t = await f(),
                                i = t.size - 24;
                            if (y(t.id, [145, 7, 220, 183, 183, 169, 207, 17, 142, 230, 0, 192, 12, 32, 83, 101])) {
                                let t = r.Buffer.alloc(16);
                                if (((i -= await e.readBuffer(t)), y(t, [64, 158, 105, 248, 77, 91, 207, 17, 168, 253, 0, 128, 95, 92, 68, 43]))) return { ext: "asf", mime: "audio/x-ms-asf" };
                                if (y(t, [192, 239, 25, 188, 77, 91, 207, 17, 168, 253, 0, 128, 95, 92, 68, 43])) return { ext: "asf", mime: "video/x-ms-asf" };
                                break;
                            }
                            await e.ignore(i);
                        }
                    ...
                    if ((await e.peekBuffer(this.buffer, { length: Math.min(256, e.fileInfo.size), mayBeLess: !0 }), this.check([97, 99, 115, 112], { offset: 36 }))) return { ext: "icc", mime: "application/vnd.iccprofile" };
                    ...
                    if (this.check([4, 0, 0, 0]) && this.buffer.length >= 16) {
                        let e = this.buffer.readUInt32LE(12);
                        if (e > 12 && this.buffer.length >= e + 16)
                            try {
                                let t = this.buffer.slice(16, e + 16).toString();
                    ...
                    if ((this.check([71]) && this.check([71], { offset: 188 })) || (this.check([71], { offset: 4 }) && this.check([71], { offset: 196 }))) return { ext: "mts", mime: "video/mp2t" };
                    ...
                    if (
                        (await e.peekBuffer(this.buffer, { length: Math.min(512, e.fileInfo.size), mayBeLess: !0 }),
                            (function (e, t = 0) {
                                let i = Number.parseInt(e.toString("utf8", 148, 154).replace(/\0.*$/, "").trim(), 8);
                    ...
                    if (this.buffer.length >= 2 && this.check([255, 224], { offset: 0, mask: [255, 224] })) {
                        if (this.check([16], { offset: 1, mask: [22] })) return this.check([8], { offset: 1, mask: [8] }), { ext: "aac", mime: "audio/aac" };
                ...
                async readTiffTag(e) {
                    let t = await this.tokenizer.readToken(e ? f : a);
                ...
                async readTiffHeader(e) {
                    let t = (e ? f : a).get(this.buffer, 2),
                        i = (e ? o : c).get(this.buffer, 4);
                    if (42 === t) {
                        if (i >= 6) {
                            if (this.checkString("CR", { offset: 8 })) return { ext: "cr2", mime: "image/x-canon-cr2" };
                            if (i >= 8 && (this.check([28, 0, 254, 0], { offset: 8 }) || this.check([31, 0, 11, 0], { offset: 8 }))) return { ext: "nef", mime: "image/x-nikon-nef" };
                        }
                        return await this.tokenizer.ignore(i), (await this.readTiffIFD(e)) ?? { ext: "tif", mime: "image/tiff" };
                    }
